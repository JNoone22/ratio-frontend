...
function App() {
  ...
  const filteredAssets = assets.filter(asset => {
    // DEBUG: Check what asset actually is
    if (search && search.toUpperCase() === 'Q') {
      console.log('🔍 ASSET TYPE:', typeof asset, 'VALUE:', asset)
    }
    
    // DEBUG: Log search state
    if (search) {
      console.log('🔍 SEARCH STATE:', search, 'Length:', search.length)
    }
    ...
    // DEBUG: Log what's happening with search
    if (search && search.toUpperCase() === 'Q') {
      if (matchesSearch) {
        console.log('✅ MATCH:', asset.symbol, '- starts with Q')
      } else {
        // This shouldn't happen but let's see if it does
        console.log('❌ NO MATCH but showing:', asset.symbol)
      }
    }
    ...
    if (search && search.toUpperCase() === 'Q') {
      console.log('📊 FINAL DECISION:', asset.symbol, 'matchesSearch:', matchesSearch, 'matchesFilter:', matchesFilter, 'RESULT:', shouldInclude)
    }
  ...
  if (search && search.toUpperCase() === 'Q') {
    console.log('🎯 RANKED ASSETS TO RENDER:', rankedAssets.length, 'assets')
    console.log('🎯 FIRST 5:', rankedAssets.slice(0, 5).map(a => a.symbol))
  }
            ...
            <div className="controls">
              <div className="search-box">
                ...
                <input
                  ...
                  value={search}
                  onChange={(e) => {
                    console.log('🔍 BIG BOARD onChange fired! Value:', e.target.value)
