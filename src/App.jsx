...
function App() {
  ...
  const [activePage, setActivePage] = useState('board') // 'board', 'grid', or 'compare'
  const [gridType, setGridType] = useState('mag7') // 'mag7', 'indices', 'crypto'
  const [gridMatchups, setGridMatchups] = useState(null)
  const [compareAsset1, setCompareAsset1] = useState(null)
  const [compareAsset2, setCompareAsset2] = useState(null)
  ...
  const filteredAssets = assets.filter(asset => {
    ...
    if (filter === 'sp500' && isSP500) {
      assetType = 'sp500'
    } else if (filter === 'ema8' && asset.above_ema8) {
      assetType = 'ema8'
    } else if (isCommodity) {
      assetType = 'commodity'
    ...
    <div className="app">
      ...
      <main className="main">
        ...
        <div className="page-toggle">
          ...
          <button 
            className={`page-toggle-btn ${activePage === 'compare' ? 'active' : ''}`}
            onClick={() => {
              setActivePage('compare')
              setCompareAsset1(null)
              setCompareAsset2(null)
            }}
          >
            ⚔️ COMPARE
          </button>
            ...
            <div className="board-container" key={filter}>
              <div className="board-header">
                ...
                <div className="col-type">
                  Type
                  <span className="info-icon" title="Asset class: Stock, ETF, or Crypto">ⓘ</span>
                </div>
                ...
                <div className="col-weeks">Weeks Trending</div>
                ...
                >
                  ...
                  <div className="col-type">
                    <span className="type-badge">
                      {['GLD', 'SLV', 'USO', 'UNG', 'CORN', 'WEAT', 'SOYB'].includes(asset.symbol) 
                        ? 'Commodity' 
          ...
          <div className="compare-container">
            <div className="compare-search-row">
              <div className="compare-search-box">
                ...
                {compareSearch1 && (
                  <div className="compare-dropdown">
                    {assets
                      .filter(a => a.symbol.toLowerCase().includes(compareSearch1.toLowerCase()))
                      .slice(0, 10)
                      .map(asset => (
                        <div
                          key={asset.symbol}
              ...
              <div className="compare-search-box">
                ...
                {compareSearch2 && (
                  <div className="compare-dropdown">
                    {assets
                      .filter(a => a.symbol.toLowerCase().includes(compareSearch2.toLowerCase()))
                      .slice(0, 10)
                      .map(asset => (
                        <div
                          key={asset.symbol}
            ...
            {compareAsset1 && compareAsset2 && (
              <div className="compare-results">
                <div className="compare-column">
                  <h2 className="compare-symbol">{compareAsset1.symbol}</h2>
                  <div className="compare-name">{compareAsset1.name}</div>
                  <div className="compare-rank">Rank #{compareAsset1.rank}</div>
                  
                  <div className="compare-stats">
                    <div className="compare-stat">
                      <div className="compare-stat-label">Tournament Record</div>
                      <div className="compare-stat-value">{compareAsset1.wins}-{compareAsset1.losses}</div>
                    </div>
                    <div className="compare-stat">
                      <div className="compare-stat-label">Win Rate</div>
                      <div className="compare-stat-value">{compareAsset1.win_rate}%</div>
                    </div>
                    <div className="compare-stat">
                      <div className="compare-stat-label">% Above 20W MA</div>
                      <div className="compare-stat-value" style={{color: getChangeColor(compareAsset1.percent_above_ma)}}>
                        {compareAsset1.percent_above_ma > 0 ? '+' : ''}{compareAsset1.percent_above_ma.toFixed(2)}%
                      </div>
                    </div>
                    <div className="compare-stat">
                      <div className="compare-stat-label">Current Price</div>
                      <div className="compare-stat-value">${compareAsset1.current_price.toFixed(2)}</div>
                    </div>
                    <div className="compare-stat">
                      <div className="compare-stat-label">20W MA</div>
                      <div className="compare-stat-value">${compareAsset1.ma.toFixed(2)}</div>
                    </div>
                    <div className="compare-stat">
                      <div className="compare-stat-label">8W EMA</div>
                      <div className="compare-stat-value">${compareAsset1.ema8?.toFixed(2) || 'N/A'}</div>
                    </div>
                    <div className="compare-stat">
                      <div className="compare-stat-label">Status</div>
                      <div className="compare-stat-value">
                        <span className={compareAsset1.above_ma ? 'status-bull' : 'status-bear'}>
                          {compareAsset1.above_ma ? '▲ Bullish' : '▼ Bearish'}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="compare-column">
                  <h2 className="compare-symbol">{compareAsset2.symbol}</h2>
                  <div className="compare-name">{compareAsset2.name}</div>
                  <div className="compare-rank">Rank #{compareAsset2.rank}</div>
                  
                  <div className="compare-stats">
                    <div className="compare-stat">
                      <div className="compare-stat-label">Tournament Record</div>
                      <div className="compare-stat-value">{compareAsset2.wins}-{compareAsset2.losses}</div>
                    </div>
                    <div className="compare-stat">
                      <div className="compare-stat-label">Win Rate</div>
                      <div className="compare-stat-value">{compareAsset2.win_rate}%</div>
                    </div>
                    <div className="compare-stat">
                      <div className="compare-stat-label">% Above 20W MA</div>
                      <div className="compare-stat-value" style={{color: getChangeColor(compareAsset2.percent_above_ma)}}>
                        {compareAsset2.percent_above_ma > 0 ? '+' : ''}{compareAsset2.percent_above_ma.toFixed(2)}%
                      </div>
                    </div>
                    <div className="compare-stat">
                      <div className="compare-stat-label">Current Price</div>
                      <div className="compare-stat-value">${compareAsset2.current_price.toFixed(2)}</div>
                    </div>
                    <div className="compare-stat">
                      <div className="compare-stat-label">20W MA</div>
                      <div className="compare-stat-value">${compareAsset2.ma.toFixed(2)}</div>
                    </div>
                    <div className="compare-stat">
                      <div className="compare-stat-label">8W EMA</div>
                      <div className="compare-stat-value">${compareAsset2.ema8?.toFixed(2) || 'N/A'}</div>
                    </div>
                    <div className="compare-stat">
                      <div className="compare-stat-label">Status</div>
                      <div className="compare-stat-value">
                        <span className={compareAsset2.above_ma ? 'status-bull' : 'status-bear'}>
                          {compareAsset2.above_ma ? '▲ Bullish' : '▼ Bearish'}
                        </span>
                      </div>
                    </div>
                  </div>
            ...
            {compareAsset1 && compareAsset2 && (
              <div className="compare-winner">
                🏆 WINNER: {compareAsset1.above_ma && !compareAsset2.above_ma ? compareAsset1.symbol : 
                           !compareAsset1.above_ma && compareAsset2.above_ma ? compareAsset2.symbol :
                           compareAsset1.rank < compareAsset2.rank ? compareAsset1.symbol : compareAsset2.symbol}
              </div>
            )}
          </div>
        )}
      ...
      <div id="start" style={{padding: '60px 20px', maxWidth: '800px', margin: '0 auto', display: activePage === 'board' ? 'block' : 'none'}}>
        <h2 style={{fontSize: '32px', marginBottom: '20px', color: '#00ff9d'}}>How Front Runner Works</h2>
        ...
        <div style={{marginBottom: '30px'}}>
          <h3 style={{fontSize: '20px', marginBottom: '10px'}}>How to Use Front Runner</h3>
          <p style={{lineHeight: '1.6', color: '#ccc'}}>
      ...
      {selectedAsset && (
        <div className="modal-overlay" onClick={() => setSelectedAsset(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="price-position-chart">
              <div className="price-position-label">Price Position</div>
              <div className="price-position-bar">
                {(() => {
                  const price = selectedAsset.current_price
                  const ema8 = selectedAsset.ema8 || selectedAsset.ma
                  const ma20 = selectedAsset.ma
                  const upper = Math.max(price, ema8, ma20) * 1.05
                  const lower = Math.min(price, ema8, ma20) * 0.95
                  const range = upper - lower
                  
                  const pricePos = ((price - lower) / range) * 100
                  const ema8Pos = ((ema8 - lower) / range) * 100
                  const ma20Pos = ((ma20 - lower) / range) * 100
                  
                  const zoneStart = Math.min(ema8Pos, ma20Pos)
                  const zoneEnd = Math.max(ema8Pos, ma20Pos)
                  
                  return (
                    <>
                      <div className="price-zone" style={{
                        left: `${zoneStart}%`,
                      ...
                      <div className="price-marker price-current" style={{left: `${pricePos}%`}}>
                        <div className="price-dot"></div>
                        <div className="price-label">${price.toFixed(2)}</div>
                      </div>
                      <div className="price-marker price-ema8" style={{left: `${ema8Pos}%`}}>
                        <div className="price-line"></div>
                        <div className="price-label">EMA8: ${ema8.toFixed(2)}</div>
                      </div>
                      <div className="price-marker price-ma20" style={{left: `${ma20Pos}%`}}>
                        <div className="price-line"></div>
                        <div className="price-label">MA20: ${ma20.toFixed(2)}</div>
                      </div>
                    </>
                  )
                })()}
