...
function App() {
  ...
  const getWinLossRecord = (symbol) => {
    ...
    if (!asset) return { wins: 0, losses: 0, winRate: 0 }
    
    const gridSymbols = getGridSymbols()
    const otherSymbols = gridSymbols.filter(s => s !== symbol)
    
    let wins = 0
    let losses = 0
    
    otherSymbols.forEach(opponentSymbol => {
      const opponent = assets.find(a => a.symbol === opponentSymbol)
      if (opponent) {
        const result = calculateHeadToHead(asset, opponent)
        if (result === 'W') wins++
        if (result === 'L') losses++
      }
    })
