...
function App() {
  ...
  const [sortBy, setSortBy] = useState('rank') // 'rank' or 'trend'
      ...
      symbols.forEach(sym2 => {
        if (sym1 === sym2) {
          ...
          if (asset1.above_ma && !asset2.above_ma) {
            matchups[sym1][sym2] = 'WIN'
          } else if (!asset1.above_ma && asset2.above_ma) {
            matchups[sym1][sym2] = 'LOSS'
          } else {
            matchups[sym1][sym2] = asset1.rank < asset2.rank ? 'WIN' : 'LOSS'
          }
        }
      })
    })
  ...
  // Apply sorting
  if (sortBy === 'trend') {
    rankedAssets = [...rankedAssets].sort((a, b) => (b.weeks_trending || 0) - (a.weeks_trending || 0))
  }
  // If sortBy === 'rank', it's already sorted by rank from the backend
          ...
          <>
            ...
            <div className="board-container" key={filter}>
              <div className="board-header">
                <div className="col-rank sortable" onClick={() => setSortBy('rank')} style={{cursor: 'pointer'}}>
                  Rank {sortBy === 'rank' && '▼'}
                </div>
                ...
                <div className="col-ma">
                  % Above MA
                  <span className="info-icon" data-tooltip="Current price vs 20-week moving average (+% = bullish, -% = bearish)">ⓘ</span>
                </div>
                <div className="col-weeks sortable" onClick={() => setSortBy('trend')} style={{cursor: 'pointer'}}>
                  Trend # {sortBy === 'trend' && '▼'}
                </div>
