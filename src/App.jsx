...
function App() {
  ...
  const getWinLossRecord = (symbol) => {
    if (gridMatchups && gridMatchups.matchups[symbol]) {
      ...
      Object.values(matchups).forEach(result => {
        if (result === 'W') wins++
        if (result === 'L') losses++
      })
                  ...
                  {gridMatchups.symbols.map(symbol => {
                    ...
                    return (
                      <div key={symbol} className="grid-stat">
                        <span className="grid-stat-symbol">{symbol}</span>
                  ...
                  <table className="grid-table">
                    ...
                    <tbody>
                      {gridMatchups.symbols.map((rowSymbol) => {
                        const rowMatchups = gridMatchups.matchups[rowSymbol]
                        if (!rowMatchups) return null
                        
                        return (
                          <tr key={rowSymbol}>
                            <td className="grid-row-label">{rowSymbol}</td>
                            {gridMatchups.symbols.map(colSymbol => {
                              const matchup = rowMatchups[colSymbol]
                              ...
                              const result = typeof matchup === 'object' ? matchup.result : matchup
                              const percent = typeof matchup === 'object' ? matchup.percent : null
